...
public class AvailableBalanceRule : IBuyRule
{
    ...
    public bool IsFollowed(ItemPage itemPage)
    {
        ...
        if (!(availableBalance < itemPage.BuyOrderPrice)) return true;
        Log.Information("Item is not profitable. Reason: no money for this item. Available balance: {0}, Price: {1}", availableBalance, itemPage.BuyOrderPrice);
        return false;
    }
}