...
public class JsonFileBasedConfigurationService : IConfigurationService
{
    #region Public

    public JsonFileBasedConfigurationService(IConfiguration configuration, string settingsPath)
    {
    ...
    public string ApiKey => _configuration.GetValue("ApiKey", string.Empty)!;
    ...
    public async Task<bool> RefreshConfigurationAsync(string apiKey, UserConfiguration userConfiguration)
    {
        ...
        var currentSettings = await File.ReadAllTextAsync(_settingsPath);
        ...
        await File.WriteAllTextAsync(_settingsPath, updatedSettings);

        if (!CheckIntegrity())
        {
            await File.WriteAllTextAsync(_settingsPath, currentSettings);
